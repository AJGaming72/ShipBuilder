...
if (_ship getVariable ["SB_isDying",false]) exitWith {systemChat "Ship cannot be Damaged: Dying";};// If we are dying, remove ability to damage
...
if (_health <= 0) then {
	...
	switch (_type) do {
		case "engine": {
			...
			systemChat(format["%1: Engine Destroyed", _ship]);
		};
		                // case "shield": {};
		                // case "cockpit": {};
	};
